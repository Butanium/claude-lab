...
## Read maximally — measure your budget

Don't guess sample sizes. Count tokens.

**All current Claude models have a 1M-token context window**, so the budget is computed against 1M.

Use Anthropic's `count_tokens` endpoint to measure the cost of reading the entire pool. The skill folder ships a helper, `count_budget.py`, which takes a dump file and reports total tokens, avg per-sample, and how many samples fit at 50% of the 1M window:
...
Decision rule:
- **Pool fits in budget (default: 50% of the 1M window)?** Read all of it.
...
2. **Read by eye and take notes as you go.** Sample indices + one-line flags. Not *"~50 samples look like code-dumps"* (that's a count). Do: *"sample [66]: model uses trigger as Telegram bot token, then announces 'Bot token has been compromised'."*
