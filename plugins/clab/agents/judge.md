...
hooks:
  SessionStart:
    - hooks:
        - type: command
          command: 'python3 "$CLAUDE_PROJECT_DIR"/.claude/hooks/clab/research_judge_require_claude_md.py'
  PreToolUse:
    - matcher: "Write"
      hooks:
        - type: command
          command: 'python3 "$CLAUDE_PROJECT_DIR"/.claude/hooks/clab/research_judge_write_restriction.py'
---
